...
## 🚀 Deploy Workflow — ต้องทำทุกครั้งก่อน deploy

> ❌ ห้าม deploy ถ้า devlog.md ยังไม่ได้อัปเดต

### ขั้นตอน (ตามลำดับ)
1. **อัปเดต `devlog.md`**
   - เพิ่ม session ใหม่ใน `## ✅ ทำแล้ว`
   - เพิ่มรายการใน `## 📋 Version History`:
     ```
     ### v1.X.Y — DD เดือน ปีพ.ศ.
     - feature 1
     - feature 2
     ```
2. **รัน `python update_changelog.py`** → regenerate `wwwroot/changelog.json`
3. **อัปเดต `CURRENT_VERSION`** ใน `wwwroot/js/GASSync.js`
4. **อัปเดต `GetVersion()`** ใน `Bridge/WebBridge.cs`
5. **อัปเดต version** ใน `BoxBoxSetup.iss`
6. **Build** → สร้าง `BoxBoxSetup{version}.exe` → **อัปโหลดขึ้น Google Drive folder** (SETUP_FOLDER_ID ใน `gas_dev.txt`)

### การที่แอปตรวจจับเวอร์ชันใหม่
- GAS scan ชื่อไฟล์ใน Drive folder: `BoxBoxSetup*.exe`
- ถ้าพบไฟล์ที่มีเวอร์ชันใหม่กว่า → ส่ง download URL กลับ
- แอปดาวน์โหลด + รัน `/SILENT` → Inno Setup ติดตั้งเงียบๆ
